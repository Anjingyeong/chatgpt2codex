...
function parseLeasePreset(value: string | boolean | undefined): LeasePreset {
  if (value === "read-only" || value === "tests-only" || value === "full-write" || value === "image-only") {
    return value;
  }
  return "full-write";
}
...
async function main(): Promise<void> {
  ...
  switch (command) {
    ...
    default:
      console.error(
        "usage: chatgpt2codex <serve|init|doctor|owner-token> [--workspace <path>] [--active-project-root <path>] [--stdio | --http [--port 7979] [--public-url <origin>]]",
      );
      process.exitCode = 1;
  }
}
