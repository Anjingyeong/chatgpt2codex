...
- **효과**: 제어 전 플랫폼·세션(X11/Wayland/헤드리스, Windows 무결성레벨·Session0)을 감지해 미지원 환경을 사유와 함께 거부. macos-dependency-doctor의 크로스플랫폼판. Windows는 UIA+SendInput(비상승 제약 문서화), Linux는 X11 우선·Wayland 포털 폴백.
